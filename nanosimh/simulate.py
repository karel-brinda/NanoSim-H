...
DEFAULT_PROFILES = [x.replace(os.path.abspath(DEFAULT_PROFILES_DIR), "")[1:-1] for x in DEFAULT_PROFILES_ABS]
...
def rnf_name(read_id, chrom_id, left, right, direction, suffix_dict={}, coord_len=0, id_len=0):
    ...
    if len(suffix_dict) == 0:
        ...
        suffix = "[{}]".format(
            ",".join(["{}:{}".format(x, suffix_dict[x] if suffix_dict[x] != "" else x) for x in suffix_dict])
        )
    return "__{}__({},{},{},{},{})__{}".format(
        str(hex_id).zfill(id_len), 1, chrom_id, direction,
        str(left).zfill(coord_len),
        str(right).zfill(coord_len), suffix
    )
...
def simulation(ref_fo, out, dna_type, per, kmer_bias, max_l, min_l, merge, rnf, rnf_cigar):
    ...
    with progressbar.ProgressBar(max_value=number_aligned) as bar:
        ...
        while i < number_aligned:
            ...
            middle, middle_ref, error_dict = error_list(
                ref, match_markov_model, match_ht_list, error_par, trans_error_pr
            )
...
def mutate_read(read, read_name, error_log, e_dict, k, aligned=True):
    ...
    for key in sorted(e_dict.keys(), reverse=True):
        ...
        if aligned and val[0] != "match":
            error_log.write(
                read_name + "\t" + str(key) + "\t" + val[0] + "\t" + str(val[1]) + "\t" + ref_base + "\t" + new_bases +
                "\n"
            )
...
def main():
    ...
    simulation(
        ref_fo=ref_fo, out=out, dna_type=dna_type, per=perfect, kmer_bias=kmer_bias, max_l=max_readlength,
